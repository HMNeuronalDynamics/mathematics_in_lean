...
def chineseMap (I : ι → Ideal R) : (R ⧸ ⨅ i, I i) →+* Π i, R ⧸ I i :=
  Ideal.quotientInfRingEquivPiQuotient I

lemma chineseMap_mk (I : ι → Ideal R) (x : R) :
    chineseMap I (Quotient.mk _ x) = fun i : ι ↦ Ideal.Quotient.mk (I i) x :=
  by simp [chineseMap, Ideal.quotientInfRingEquivPiQuotient]
...
lemma chineseMap_inj (I : ι → Ideal R) : Injective (chineseMap I) := by
  rw [← injective_lift_iff, ← Function.Injective.comp_iff]
  exact Ideal.quotientInfRingEquivPiQuotient.injective
...
theorem isCoprime_Inf {I : Ideal R} {J : ι → Ideal R} {s : Finset ι}
  ...
  | @insert i s _ hs =>
      ...
      set K := ⨅ j ∈ s, J j
      calc
        1 = I + K                  := hs (λ j hj => hf j (Finset.mem_insert_of_mem hj))
        _ = I + K * (I + J i)      := by rw [hf i (Finset.mem_insert_self i s), add_comm]
        _ = (1 + K) * I + K * J i  := by rw [mul_add, add_mul, add_comm, ← mul_assoc, ← mul_assoc]
        _ ≤ I + K ⊓ J i            := by rw [mul_add, add_comm, ← mul_assoc, ← mul_assoc]; exact add_le_add le_sup_left le_sup_right
...
lemma chineseMap_surj [Fintype ι] {I : ι → Ideal R}
  ...
  have key : ∀ i, ∃ e : R, mk (I i) e = 1 ∧ ∀ j, j ≠ i → mk (I j) e = 0 := by
    ...
    have hI' : ∀ j ∈ ({i} : Finset ι)ᶜ, IsCoprime (I i) (I j) := by
      intro j hj
      apply hI
      exact Finset.not_mem_singleton.mp (Finset.mem_compl.mp hj)
    simp_rw [← isCoprime_iff_sup_eq, ← isCoprime_iff_codisjoint] at hI'
    rcases hI' (Finset.mem_compl_self i) with ⟨a, ha, b, hb, hab⟩
    use a, ha, b, λ j hj => hb j (Finset.not_mem_singleton.mp hj), hab

  choose e he using key
  use mk _ (∑ i, f i * e i)
  ext i
  simp [chineseMap_mk', he, hf]
