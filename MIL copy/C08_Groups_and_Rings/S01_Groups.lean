...
def conjugate {G : Type*} [Group G] (x : G) (H : Subgroup G) : Subgroup G where
  ...
  mul_mem' := by
    dsimp
    rintro a b ⟨h, hh, rfl⟩ ⟨k, hk, rfl⟩
    use h * k
    simp [hh, hk, mul_assoc]
...
example (φ : G →* H) (S T : Subgroup H) (hST : S ≤ T) : comap φ S ≤ comap φ T := by
  exact λ g hg => hST (comap φ S g hg)

example (φ : G →* H) (S T : Subgroup G) (hST : S ≤ T) : map φ S ≤ map φ T := by
  exact λ h ⟨g, hg, rfl⟩ => ⟨g, hST hg, rfl⟩
...
example (φ : G →* H) (ψ : H →* K) (U : Subgroup K) :
  ...
  ext
  simp [comap, MonoidHom.comp_apply]
...
lemma eq_bot_iff_card {G : Type*} [Group G] {H : Subgroup G} [Fintype H] :
  ...
  exact ⟨λ h x hx => ⟨x, hx, h⟩, λ ⟨x, hx, h⟩ y hy => h y hy⟩

lemma inf_bot_of_coprime {G : Type*} [Group G] (H K : Subgroup G) [Fintype H] [Fintype K]
    (h : (card H).Coprime (card K)) : H ⟓ K = ⊥ := by
  rw [← eq_bot_iff_card, card_eq_one_iff]
  have : card (H ⟓ K) ∣ card H ∧ card (H ⟓ K) ∣ card K := ⟨card_dvd_of_le inf_le_left, card_dvd_of_le inf_le_right⟩
  obtain ⟨d, hd⟩ := (Nat.dvd_antisymm (this.1.trans h.dvd_right) this.2).trans hd.symm,
  use 1
  rw [hd, mul_one]
...
instance : MulAction G (Subgroup G) where
  ...
  mul_smul := λ g g' H => by
    ext x
    simp [conjugate, mul_assoc, inv_mul_cancel_left]
...
lemma aux_card_eq [Fintype G] (h' : card G = card H * card K) : card (G ⧸ H) = card K := by
  have := (Subgroup.index_eq_card H).symm
  rw [this, ←h', Nat.mul_div_cancel_left _ (card_pos H).ne']
...
def iso₁ [Fintype G] (h : Disjoint H K) (h' : card G = card H * card K) : K ≃* G ⧸ H := by
  have := aux_card_eq h'
  exact Equiv.ofBijective _ ⟨ker_eq_bot_iff.2 _, by simp [this]⟩

def iso₂ : G ≃* (G ⧸ K) × (G ⧸ H) := by
  refine Equiv.ofBijective _ ⟨ker_eq_bot_iff.2 _, _⟩
  sorry

#check MulEquiv.prodCongr

def finalIso : G ≃* H × K :=
  MulEquiv.prodCongr (iso₁ h h') (iso₂)
