...
theorem sb_injective (hf : Injective f) : Injective (sbFun f g) := by
  ...
  have : invFun g x₁ = invFun g x₂ := by
    ...
    exact hxeq
  exact (leftInverse_invFun hg).inj this
...
theorem sb_surjective (hf : Injective f) (hg : Injective g) : Surjective (sbFun f g) := by
  ...
  · rw [A_def, sbSet, mem_iUnion] at gyA
    ...
    exact hg hx
  use invFun g y
  simp only [h_def, sbFun, if_neg]
  · exact (leftInverse_invFun hg y).symm
  intro h
  rw [A_def, sbSet, mem_iUnion] at h
  rcases h with ⟨n, hn⟩
