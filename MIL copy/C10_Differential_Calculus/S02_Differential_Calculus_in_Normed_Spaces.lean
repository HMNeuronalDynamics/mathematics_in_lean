...
example {ι : Type*} [CompleteSpace E] {g : ι → E →L[𝕜] F} (h : ∀ x, ∃ C, ∀ i, ‖g i x‖ ≤ C) :
  ...
  have hc : ∀ n : ℕ, IsClosed (e n) :=
    λ n, isClosed_iInter (λ i, isClosed_le (ContinuousLinearMap.continuous _) continuous_const)
  ...
  have hU : (⋃ n : ℕ, e n) = univ :=
    by
      ...
      exact ⟨m, λ i, (hC i).trans (mul_le_mul_of_nonneg_right hm (norm_nonneg x))⟩
  ...
  obtain ⟨ε, ε_pos, hε⟩ : ∃ ε > 0, ball x ε ⊆ interior (e m) := mem_interior_iff_mem_nhds.mp hx
  obtain ⟨k, hk⟩ : ∃ k : 𝕜, 1 < ‖k‖ := NormedField.exists_one_lt_norm 𝕜
  -- show all elements in the ball have norm bounded by `m` after applying any `g i`
  have real_norm_le : ∀ z ∈ ball x ε, ∀ (i : ι), ‖g i z‖ ≤ m :=
    λ z hz i, interior_subset (hε hz) i
  have εk_pos : 0 < ε / ‖k‖ := div_pos ε_pos (norm_pos_iff.mpr (ne_of_gt hk))
  refine ⟨(m + m : ℕ) / (ε / ‖k‖), λ i ↦ ContinuousLinearMap.opNorm_le_of_shell ε_pos _ hk _⟩
  · rwa [interior_eq_nhds] at hx
  · intros z hz
    obtain ⟨c, rfl⟩ := NormedAddCommGroup.exists_smul_mem_ball_of_one_lt_norm z x ε_pos hk hz
    exact (f.le_op_norm c).trans (mul_le_mul_of_nonneg_left (real_norm_le _ hz i) (norm_nonneg c))
  · rw [interior_eq_nhds] at hx
    intros z hz
    obtain ⟨c, rfl⟩ := NormedAddCommGroup.exists_smul_mem_ball_of_one_lt_norm z x ε_pos hk hz
    exact (f.le_op_norm c).trans (mul_le_mul_of_nonneg_left (real_norm_le _ hz i) (norm_nonneg c))
