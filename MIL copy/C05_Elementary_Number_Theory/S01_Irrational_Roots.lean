...
example {m n : ℕ} (coprime_mn : m.Coprime n) : m ^ 2 ≠ 2 * n ^ 2 := by
  ...
  have : 2 * (2 * k ^ 2) = 2 * n ^ 2 := by
    ...
    ring
  have : 2 * k ^ 2 = n ^ 2 :=
    (mul_right_inj' (by norm_num : 2 ≠ 0)).mp this
  have : 2 ∣ n := by
    apply even_of_even_sqr
    rw [this]
    exact dvd_mul_left 2 (k ^ 2)
  have : 2 ∣ m.gcd n := by
    apply Nat.dvd_gcd
    · exact dvd_iff_exists_eq_mul_left.mpr ⟨k, meq⟩
    · exact this
  have : 2 ∣ 1 := by
...
example {m n p : ℕ} (nnz : n ≠ 0) (prime_p : p.Prime) : m ^ 2 ≠ p * n ^ 2 := by
  ...
  have eq2 : (p * n ^ 2).factorization p = 2 * n.factorization p + 1 := by
    rw [Nat.factorization_mul prime_p.ne_zero nsqr_nez, prime_p.factorization', factorization_pow' n 2 p]
    ring
...
example {m n k r : ℕ} (nnz : n ≠ 0) (pow_eq : m ^ k = r * n ^ k) {p : ℕ} (prime_p : p.Prime) :
  ...
  have eq2 : (r.succ * n ^ k).factorization p = k * n.factorization p + r.succ.factorization p := by
    rw [Nat.factorization_mul r.succ_ne_zero npow_nz, factorization_pow' n k p]
  have : r.succ.factorization p = k * m.factorization p - k * n.factorization p := by
    rw [← eq1, pow_eq, eq2, add_comm, Nat.add_sub_cancel]
  rw [this]
  exact dvd_mul_right k (m.factorization p - n.factorization p)
