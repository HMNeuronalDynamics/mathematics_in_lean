...
theorem pow_two_le_fac (n : ℕ) : 2 ^ (n - 1) ≤ fac n := by
  ...
  induction' n with n ih
  · simp [fac]
  · rw [Nat.succ_sub_succ_eq_sub, Nat.sub_zero] at ih
    rw [Nat.succ_eq_add_one, pow_succ, fac]
...
theorem sum_sqr (n : ℕ) : ∑ i in range (n + 1), i ^ 2 = n * (n + 1) * (2 * n + 1) / 6 := by
  ...
  induction' n with n ih
  · simp
  rw [Finset.sum_range_succ, Nat.succ_eq_add_one, mul_add, add_mul, add_mul, add_mul, ih]
  ring
...
theorem mul_add (m n k : MyNat) : mul m (add n k) = add (mul m n) (mul m k) := by
  induction' k with k ih
  · rw [add, mul, mul, zero_add]
  rw [add, mul, mul, ih, add_assoc, add_assoc]
  congr 1
  rw [add_comm (mul m k), add_assoc]
...
theorem succ_mul (m n : MyNat) : mul (succ m) n = add (mul m n) n := by
  induction' n with n ih
  · rw [mul, mul, add]
  rw [mul, mul, ih, succ_add, add_assoc]

theorem mul_comm (m n : MyNat) : mul m n = mul n m := by
  induction' n with n ih
  · rw [mul, zero_mul]
  rw [mul, succ_mul, ih, add_comm]
