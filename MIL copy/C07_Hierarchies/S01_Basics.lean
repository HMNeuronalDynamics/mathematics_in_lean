...
lemma dia_inv [Group₁ G] (a : G) : a ⋄ a⁻¹ = 𝟙 := by
  rw [← inv_dia a⁻¹, ← dia_assoc, inv_dia, dia_one]
...
lemma left_inv_eq_right_inv' {M : Type} [Monoid₃ M] {a b c : M} (hba : b * a = 1) (hac : a * c = 1) : b = c := by
  rw [← one_mul c, ← hba, mul_assoc₃, hac, mul_one]
...
lemma inv_eq_of_mul [Group₃ G] {a b : G} (h : a * b = 1) : a⁻¹ = b :=
  (left_inv_eq_right_inv' (inv_mul a) h).symm

@[to_additive (attr := simp)]
lemma Group₃.mul_inv {G : Type} [Group₃ G] {a : G} : a * a⁻¹ = 1 := inv_mul a

@[to_additive]
lemma mul_left_cancel₃ {G : Type} [Group₃ G] {a b c : G} (h : a * b = a * c) : b = c := by
  have h1 : a⁻¹ * (a * b) = a⁻¹ * (a * c) := congr_arg (· * a⁻¹) h
  rwa [← mul_assoc₃, inv_mul, one_mul, ← mul_assoc₃, inv_mul, one_mul] at h1

@[to_additive]
lemma mul_right_cancel₃ {G : Type} [Group₃ G] {a b c : G} (h : b * a = c * a) : b = c := by
  have h1 : (b * a) * a⁻¹ = (c * a) * a⁻¹ := congr_arg (· * a⁻¹) h
  rwa [mul_assoc₃, mul_inv, mul_one, mul_assoc₃, mul_inv, mul_one] at h1
...
{ Ring₃.toAddGroup₃ with
  add_comm := by intros; apply add_comm }
...
instance abGrpModule (A : Type) [AddCommGroup₃ A] : Module₁ ℤ A where
  smul := zsmul₁
  zero_smul := by intros; simp [zsmul₁, nsmul₁]
  one_smul := by intros; simp [zsmul₁, nsmul₁]
  mul_smul := by intros; cases a; simp [zsmul₁, nsmul₁, mul_smul]
  add_smul := by intros; cases a; simp [zsmul₁, nsmul₁, add_smul]
  smul_add := by intros; simp [zsmul₁, nsmul₁, smul_add]
