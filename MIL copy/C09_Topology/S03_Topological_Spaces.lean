...
example {α : Type*} (n : α → Filter α) (H₀ : ∀ a, pure a ≤ n a)
  ...
  by
    intros a s hs
    have : ∀ᶠ y in n a, ∀ᶠ x in n y, x ∈ s := H a s hs
    filter_upwards [this] with y hy using ⟨n y, hy, subset.refl _, fun x hx => hy x hx⟩
...
theorem aux {X Y A : Type*} [TopologicalSpace X] {c : A → X}
  ...
  rw [Tendsto] at h
  specialize h V' V'_in
  rcases mem_comap.mp h with ⟨U, hU, h⟩
  exact ⟨U, hU, is_open_iff_mem_nhds.mpr hU, h⟩
...
example [TopologicalSpace X] [TopologicalSpace Y] [T3Space Y] {A : Set X}
  ...
  refine ⟨φ, fun a => hφ a, fun a => _⟩
  have : Tendsto (fun x : X ↦ if h : x ∈ A then f ⟨x, h⟩ else φ x) (𝓝 a) (𝓝 (φ a)) := by
    apply continuous_if.2
    exact ⟨f_cont.continuous_on, fun x hx => hφ x⟩
  exact this.mono_left nhds_within_le_nhds
...
example [TopologicalSpace Y] {f : X → Y} (hf : Continuous f) {s : Set X} (hs : IsCompact s) :
    IsCompact (f '' s) := by
  intro F F_ne F_le
  have map_eq : map f (𝓟 s ⊓ comap f F) = 𝓟 (f '' s) ⊓ F := by rw [map_inf_principal_preimage, map_comap_eq]
  have Hne : (𝓟 s ⊓ comap f F).NeBot := (inf_ne_bot_iff.2 ⟨principal_ne_bot_iff.2 ⟨s, Subset.refl s⟩, F_ne⟩)
  have Hle : 𝓟 s ⊓ comap f F ≤ 𝓟 s := inf_le_left
  rcases hs Hne Hle with ⟨a, ha, haF⟩
  exact ⟨f a, mem_image_of_mem f ha, ⟨f a, ha, haF⟩⟩
