...
theorem cauchySeq_of_le_geometric_two' {u : ℕ → X}
  ...
  obtain ⟨N, hN⟩ : ∃ N : ℕ, 1 / 2 ^ N * 2 < ε := by
    have : tendsto (fun N => 2 * (1 / 2) ^ N) atTop (𝓝 0) := tendsto_const_nhds.mul tendsto_pow_atTop_nhds_0_of_lt_1
      (by norm_num)
    exact ((tendsto_order.1 this).2 ε ε_pos).exists
  use N
  intro n hn
  obtain ⟨k, rfl : n = N + k⟩ := le_iff_exists_add.mp hn
  calc
    dist (u (N + k)) (u N) = dist (u (N + 0)) (u (N + k)) := by rw [add_zero]
    _ ≤ ∑ i in range k, dist (u (N + i)) (u (N + (i + 1))) := dist_le_range_sum_dist
    _ ≤ ∑ i in range k, (1 / 2 : ℝ) ^ (N + i) := sum_le_sum fun i _ => hu (N + i)
    _ = 1 / 2 ^ N * ∑ i in range k, (1 / 2 : ℝ) ^ i := by rw [sum_geometric]
    _ ≤ 1 / 2 ^ N * 2 := mul_le_mul_of_nonneg_left (sum_geometric_two_le _) (one_div_pos.2 (pow_pos (by norm_num) _)).le
    _ < ε := hN
...
example [CompleteSpace X] (f : ℕ → Set X) (ho : ∀ n, IsOpen (f n)) (hd : ∀ n, Dense (f n)) :
  ...
  have :
    ...
    by
      intro n x δ δpos
      obtain ⟨y, hy⟩ : (f n ∩ closedBall x δ).Nonempty := (hd n).inter_open_nonempty (ho n) δpos
      obtain ⟨r, rpos, hr⟩ := isOpen_iff.1 (ho n) y hy.1
      refine ⟨y, min r (B (n + 1)), (min_pos rpos (Bpos (n + 1))), le_min (le_refl _) (le_of_lt (Bpos _)), ?_⟩
      exact fun z hz => ⟨hr (ball_subset_closedBall hz) hy.1, ball_subset_closedBall hz hy.2⟩
  ...
  have rpos : ∀ n, 0 < r n := by
    intro n
    induction n
    case zero => exact lt_min εpos (Bpos 0)
  ...
  exact ⟨yball 0, fun n => (incl n).2 (yball n.succ)⟩
